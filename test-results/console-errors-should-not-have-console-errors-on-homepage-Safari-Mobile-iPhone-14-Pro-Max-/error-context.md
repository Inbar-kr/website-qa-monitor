...
- Name: should not have console errors on homepage
- Location: C:\Users\ענבר\VSCodeProjects\website-qa-monitor\tests\console-errors.spec.ts:3:5
...
Expected: 0
Received: 2
    at C:\Users\ענבר\VSCodeProjects\website-qa-monitor\tests\console-errors.spec.ts:19:32
```
...
```ts
   ...
   2 |
   3 | test('should not have console errors on homepage', async ({ page }) => {
  ...
  12 |   await page.goto('/');
  13 |
  14 |   if (consoleErrors.length > 0) {
  15 |     console.log('Console errors found:');
  16 |     consoleErrors.forEach(err => console.log(err));
  17 |   }
  18 |
> 19 |   expect(consoleErrors.length).toBe(0);
     |                                ^ Error: expect(received).toBe(expected) // Object.is equality
  20 | });
  21 |
```